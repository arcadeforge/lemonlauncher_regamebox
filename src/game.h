...
private:
   ...
   string _orientation; // game orientation v / h

public:
 game(const char* rom, const char* name, const char* params, const char* orientation) :
   _rom(rom), _name(name), _params(params), _orientation(orientation) { }
   ...
   /** Returns Game orientation (if any) */
   const char* orientation() const
   { return _orientation.c_str(); }
