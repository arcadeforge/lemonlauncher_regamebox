...
void options::load(const char* conf_dir)
{
   ...
   cfg_opt_t opts[] = {
      ...
      CFG_INT(KEY_SCREEN_WIDTH, 320, CFGF_NONE),
      CFG_INT(KEY_SCREEN_HEIGHT, 240, CFGF_NONE),
      CFG_INT(KEY_SCREEN_BPP, 16, CFGF_NONE),
      CFG_BOOL(KEY_FULLSCREEN, cfg_false, CFGF_NONE),
      CFG_STR(KEY_ORIENTATIONFILTER, "", CFGF_NONE),

      CFG_STR(KEY_VIDEO_X1, "0", CFGF_NONE),
      CFG_STR(KEY_VIDEO_X2, "0", CFGF_NONE),
      CFG_STR(KEY_VIDEO_Y1, "0", CFGF_NONE),
      CFG_STR(KEY_VIDEO_Y2, "0", CFGF_NONE),
      CFG_STR(KEY_VIDEO_OR, "0", CFGF_NONE),

      // CM
      CFG_STR(KEY_PI2JAMMAJOYSTICK, "y", CFGF_NONE),
      CFG_INT(KEY_PI2JAMMAJOYSTICK_DELAY, 150, CFGF_NONE),

      CFG_STR(KEY_REMEMBERLAST, "y", CFGF_NONE),
      CFG_STR(KEY_REMEMBERLASTFILE, "/tmp/ll_rememberlastselection_tmp_file.txt", CFGF_NONE),
      ...
      CFG_INT(KEY_KEYCODE_FAV, 257, CFGF_NONE),

      // setting to 99 and 98 for default to prevent thumb stick 
      // interference
      CFG_INT(JOY_UP, 99, CFGF_NONE),
      CFG_INT(JOY_DOWN, 99, CFGF_NONE),
      CFG_INT(JOY_PGUP, 98, CFGF_NONE),
      CFG_INT(JOY_PGDOWN, 98, CFGF_NONE),
      CFG_INT(JOY_BUTTON_UP, 13, CFGF_NONE),
      CFG_INT(JOY_BUTTON_DOWN, 14, CFGF_NONE),
      CFG_INT(JOY_BUTTON_PGUP, 15, CFGF_NONE),
      CFG_INT(JOY_BUTTON_PGDOWN, 16, CFGF_NONE),
      CFG_INT(JOY_SELECT, 0, CFGF_NONE),
      CFG_INT(JOY_BACK, 1, CFGF_NONE),
      CFG_INT(JOY_ALPHAMOD_DOWN, 5, CFGF_NONE),
      CFG_INT(JOY_ALPHAMOD_UP, 4, CFGF_NONE),
      CFG_INT(JOY_FAV, 3, CFGF_NONE),
